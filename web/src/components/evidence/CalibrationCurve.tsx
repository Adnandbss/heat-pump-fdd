...
export function CalibrationCurve({ data }: Props) {
  ...
  const yMin = yValues.length ? Math.min(...yValues) * 0.92 : 0;
  const yMax = 1;
  ...
  return (
    <GlassCard className="p-6" data-testid="calibration-curve">
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <ProtocolBadge protocol={points[0]?.protocol} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {facets.map((facet) => {
          ...
          return (
            <div key={facet} className="rounded-2xl border border-white/10 p-3">
              ...
              <div className="h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chart} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="TTdew" stroke="#2DD4BF" strokeWidth={2} dot={{ r: 2 }} connectNulls />
                    {showAnn && ann0 ? (
      ...
      <div className="flex gap-4 text-[11px] text-white/45 mt-3">
        <span className="inline-flex items-center gap-1.5">
          <span className="h-1.5 w-4 rounded-full bg-[#A78BFA]" /> TT
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-1.5 w-4 rounded-full bg-[#2DD4BF]" /> TTdew
        </span>
